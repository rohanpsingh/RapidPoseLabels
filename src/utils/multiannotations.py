...
class MultiAnnotations:
    def __init__(self, scene_path, input_arr_path, input_model_path, visualize=False):
        """
        ...
        self.input_arrays = [np.load(path) for path in input_arr_path]
        self.model_paths  = input_model_path
    ...
    def visualize_sample(self, sample, color):
        """
        ...
        #draw keypoints
        for point in keypts:
            cv2.circle(input_img, tuple(map(int, point)), 5, color, -1)
        #draw bounding-box
        '''
        ...
        cv2.drawContours(input_img, contours, 0, color, -1)
        #draw bounding-box
    ...
    def process_input(self):
        ...
        for input_arr in self.input_arrays:
            ...
            out_ts  = input_arr['scenes'][ :(num_scenes)*3].reshape((num_scenes, 3))
            out_qs  = input_arr['scenes'][(num_scenes)*3 : (num_scenes)*7].reshape((num_scenes, 4))
        ...
        for obj in self.model_paths:
            ...
            dense_model = o3d.io.read_point_cloud(os.path.join(os.path.dirname(obj), "dense.ply"))
            self.object_model.append([sparse_model, np.asarray(dense_model.points)])
        return
    ...
    def generate_labels(self):
        ...
        with open(os.path.join(self.dataset_path, 'associations.txt'), 'r') as file:
            img_name_list = file.readlines()

        #read the camera pose corresponding to each frame
        with open(os.path.join(self.dataset_path, 'camera.poses'), 'r') as file:
        ...
        for im_path, cam_pose in zipped_list:
            ...
            for idx in range(len(self.scene_tfs)):
                ...
                cam_t = tfa.compose(np.asarray(cam_pose[:3]), tfq.quat2mat(np.asarray([cam_pose[-1]] + cam_pose[3:-1])),
                                    np.ones(3))
                #get 2D positions of keypoints, centers and scale of bounding box
                label.append(self.project_points(obj_model, np.dot(np.linalg.inv(cam_t), scene_tf)))
